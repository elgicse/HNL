...
def sensitivityScanTLEP(model=2, Rmin=1.e-3, Rmax=1., nZ=1.e12, ndivx=220, ndivy=400):
    verbose = True
    ...
    grShipFile = r.TFile('out/plots/SHiP-graphs-model%s.root'%(model), 'read')
    ...
    filecontuttodentro = r.TFile('out/plots/perNicola/SHiPupdated-model%s_%s_%sZ_%s_%s.root'%(model,h,nZ,Rmin,Rmax),'recreate')
    mgr.Write()
    c1.Write()
    grShip.Write()
    grbbn.Write()
...
def merge1213nh():
    f13 = r.TFile('out/plots/perNicola/SHiPupdated-model2_normal_1e+13Z_0.0001_5.0.root','read')
    f12 = r.TFile('out/plots/perNicola/SHiPupdated-model2_normal_1e+12Z_0.001_1.0.root','read')
    ...
    leg = r.TLegend(0.2034,0.8228,0.8234,0.9983)
...
"""
def merge1213cms():
    f13 = r.TFile('out/plots/perNicola/CMS-model2_normal_1e+11Z_0.01_1.0.root','read')
    f12 = r.TFile('out/plots/perNicola/CMS-model2_normal_1e+11Z_0.1_1.0.root','read')
    #fShip = r.TFile('out/plots/SHiP-graphs-model2.root','read')
    mgr = f12.Get('mgr_inverted_1e+11')
    gr13 = r.TGraph(f13.Get('TLEP'))
    gr13.SetLineStyle(9)
    mgr.Add(gr13)
    r.gStyle.SetOptTitle(0)
