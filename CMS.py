...
#stop = [math.log10(43.)]*3 #GeV # works for 1cm
stop = [math.log10(28.)]*3 #GeV # works for 10cm
...
def sensitivityScanTLEP(model=2, Rmin=1.e-3, Rmax=1., nW=1.e12, ndivx=200, ndivy=400):
    ...
    filecontuttodentro = r.TFile('out/plots/perNicola/CMS-SHiPupdated-model%s_%s_%sW_%s_%s.root'%(model,h,nW,Rmin,Rmax),'recreate')
    mgr.Write()
    c1.Write()
    grShip.Write()
    grbbn.Write()
...
def merge1213nh():
    f13 = r.TFile('out/plots/perNicola/CMS-SHiPupdated-model2_normal_1e+11W_0.01_1.0.root','read')
    f12 = r.TFile('out/plots/perNicola/CMS-SHiPupdated-model2_normal_1e+11W_0.1_1.0.root','read')
    mgr = f12.Get('mgr_normal_1e+11')
    ...
    leg = r.TLegend(0.2048,0.8228,0.7966,0.9966)
    ...
    labels.DrawLatexNDC(0.61, 0.69, '#it{BAU}')
